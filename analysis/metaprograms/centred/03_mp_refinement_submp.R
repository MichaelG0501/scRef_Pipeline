...
# ============================================================================
# 5. Data-driven MP grouping (centred workflow — no hardcoded state assignments)
# ============================================================================

# Since centred NMF produces a new MP numbering, we cannot use hardcoded
# state_groups. Instead, all non-removed MPs participate in refinement.
...
# ============================================================================
# 8. Build refined MP set for final plots (data-driven, all non-removed MPs)
# ============================================================================

# Build the ordered refined MP list from keep + split results
...
# Display labels: use MP name as-is (no hardcoded descriptions for centred workflow)
...
for (mp in split_mps) {
  ...
  common_cells <- Reduce(intersect, list(rownames(refined_ucell), Cells(tmdata_all)))
  ...
  # Only one group now (the MP itself)
...
# Split factor: one level per parent MP
split_factor <- factor(
  mp_to_parent,
  levels = unique(mp_to_parent)
)
