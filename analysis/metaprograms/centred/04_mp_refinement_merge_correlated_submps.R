...
# Placeholder for state groupings — will be updated after enrichment annotation
state_groups <- list()

mp_desc_map <- c()
submp_desc_map <- c()

# Helper function to assign colours dynamically
...
if (!replot_only && (force_rebuild || !gene_cache_valid)) {
  ...
  if (length(plus_features) > 0) {
    ...
    geneNMF.programs <- readRDS("Metaprogrammes_Results/centred/geneNMF_outs.rds")
...
metrics_list <- lapply(refined_mps_list, function(mp) {
  ...
  if (n_progs < 2) {
    return(data.frame(
      sampleCoverage = NA_real_,
      meanCosineSim = NA_real_,
      medianCosineSim = NA_real_,
      Q10CosineSim = NA_real_,
      minCosineSim = NA_real_,
      programConsistency = NA_real_,
      numberGenes = n_genes,
      numberPrograms = n_progs,
      row.names = mp,
      stringsAsFactors = FALSE
    ))
  }
  ...
  prog_mean_sims <- sapply(seq_len(n_progs), function(i) {
    mean(J_sub[i, -i])
  })
  ...
  data.frame(
    sampleCoverage = sample_cov,
    meanCosineSim = mean_cos,
    medianCosineSim = median_cos,
    Q10CosineSim = q10_cos,
    minCosineSim = min_cos,
    programConsistency = prog_consistency,
    numberGenes = n_genes,
    numberPrograms = n_progs,
    row.names = mp,
    stringsAsFactors = FALSE
...
ord <- order(metaprograms.metrics$sampleCoverage, metaprograms.metrics$meanCosineSim, decreasing = TRUE)
metaprograms.metrics <- metaprograms.metrics[ord, ]

out_rds <- file.path(outdir, "tables", "merged_refined_mp_metrics.rds")
out_csv <- file.path(outdir, "tables", "merged_refined_mp_metrics.csv")
saveRDS(metaprograms.metrics, out_rds)
write.csv(metaprograms.metrics, out_csv)
cat("Saved:", out_rds, "and", out_csv, "\n")
...
if (length(full_plot_features) >= 2) {
  ...
  ht_full_plot <- Heatmap(full_plot_display_rho,
    ...
    row_title = NULL,
    column_title = "Ordered: All Merged sub-MPs",
...
####################
# Use unsupervised clustering order for enrichment
hc <- hclust(dist(mean_rho))
final_col_order <- colnames(mean_rho)[hc$order]
heatmap_gaps_col <- 0
####################
...
out_pdf_merged <- file.path(outdir, "figures", "New_nMP_optimal_anno_merged.pdf")
run_enrichment_and_plot(merged_mp_genes, NULL, NULL, out_pdf_merged, cols_palette)
cat("Finished merged enrichment annotation.\n")
