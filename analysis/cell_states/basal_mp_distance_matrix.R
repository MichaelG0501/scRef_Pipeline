...
# Monocle3-based MP-to-MP distance comparison for the 5 MPs
# defining the Basal to Intestinal Metaplasia state (noreg Approach B).
#
# For each Basal cell, its dominant MP is assigned from {MP17,MP14,MP5,MP10,MP8}.
# Hybrid: when gap between top-1 and top-2 MP z-scores < 0.3 (same as state def).
#
...
mp_labels <- c(
  MP17 = "MP17 Basal-like Transition",
  MP14 = "MP14 Hypoxia Adapted Epi.",
  MP5  = "MP5 Epithelial IFN Resp.",
  MP10 = "MP10 Columnar Diff.",
  MP8  = "MP8 Intestinal Diff."
)
mp_cols <- c(
  MP17 = "#4DAF4A", MP14 = "#8DA0CB", MP5 = "#66C2A5",
  MP10 = "#A6D854", MP8 = "#FC8D62", Hybrid = "black"
)
HYBRID_GAP <- 0.1
ROOT_MIN <- 15
OTHER_MIN <- 15
...
# Use MP17 as default root (most "basal" MP)
ROOT_MP <- "MP17"
        ...
        if (!is.null(ord_cds)) {
          ...
          if (cur_root == ROOT_MP) {
            ...
            p1 <- plot_cells(ord_cds, color_cells_by="pseudotime", show_trajectory_graph=TRUE,
              ...
              scale_color_viridis_c() + labs(title=paste0(sid, " | Pseudotime (MP17 root)")) + theme_minimal(base_size=11)
