...
#   Description: Score the final centred 17-MP signatures in the Yates Xenium
#     cohort and assign the five current noreg Approach B states.
#   Inputs: external Yates Zenodo Xenium matrices/sample metadata plus
#     ref_outs/xenium_scatlas_states/Auto_scATLAS_{mp_gene_ranked,state_groups,mp_signature_summary,mp_order}.csv.
#   Outputs: ref_outs/xenium_scatlas_states/ cell annotation CSV.GZ, threshold/
#     overlap/abundance tables, parameter TXT, and slide-readable PDF/PNG figures.
#   Cache/replot: deterministic from exported signatures and external matrices; no hidden cache.
#   Run: qsub analysis/spatial/map_scatlas_states_xenium.sh
#   Conda env: bidcell_temp (mapping); dmtcp (signature export in wrapper)
####################
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        default="/rds/general/project/tumourheterogeneity1/live/scRef_Pipeline/ref_outs/xenium_scatlas_states",
    )
    parser.add_argument(
        "--signature-dir",
        default=None,
