...
# Analysis registry:
#   Status: legacy
#   Script: analysis/publication/legacy_poster_section7_survival_targeting.R
...
make_km_curve <- function(states, output_name) {
  ...
  if (!all(file.exists(expr_file, meta_file, SCREF_LEGACY_MP_OBJECT_RDS, cox_file))) return(NULL)
  ...
  geneNMF.metaprograms <- readRDS(SCREF_LEGACY_MP_OBJECT_RDS)
